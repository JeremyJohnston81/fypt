...
export default async function createReport(subjectProperty, comps, action = 'open') {
	...
	const subjectPropertyAdjustedPrice =
		subjectProperty.sqFt * compAvgSqFtPrice + subjectProperty.landValue + subjectProperty.xFeatures;
	const taxSavings =
		(subjectProperty.marketValue - subjectPropertyAdjustedPrice) * (subjectProperty.taxRate / 100) <
		0
			? 0
	...
	let mapURL =
		'https://maps.googleapis.com/maps/api/staticmap?scale=2&size=600x300&maptype=satellite';
	...
	subjectDataRow.push(
		subjectProperty.imprvVal.toFixed().replace(/(\d)(?=(\d{3})+(,|$))/g, '$1,') +
			'\n' +
			subjectProperty.xFeatures.toFixed().replace(/(\d)(?=(\d{3})+(,|$))/g, '$1,')
	);
	subjectDataRow.push(
		subjectProperty.marketValue.toFixed().replace(/(\d)(?=(\d{3})+(,|$))/g, '$1,')
	);
	...
	comps.forEach(function (sourceRow) {
		...
		dataRow.push(
			sourceRow.imprvVal.toFixed().replace(/(\d)(?=(\d{3})+(,|$))/g, '$1,') +
				'\n' +
	...
	const docDefinition = {
			...
			[
				{
					...
					text:
						'Report Generated ' +
						new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '').substr(0, 10),
					style: 'coverPage',
					marginBottom: 20
				}
			]
		...
		content: [
			{
				text: 'Property Tax Comparable Report',
				style: 'coverPage',
				fontSize: 20,
				marginBottom: 10,
				marginTop: 130
			...
			{ text: subjectProperty.address, style: 'coverPage', marginBottom: 10 },
			{
				...
				table: {
					...
					body: [
						...
						[
							{ text: 'Condition, Desirability, and Utility (CDU):' },
							{ text: subjectProperty.cduDesc }
						],
						[{ text: 'Grade:' }, { text: subjectProperty.grdDesc }],
						[
							{ text: 'Appraised Market Value:' },
							{
								text:
									'$' +
									subjectProperty.marketValue.toFixed().replace(/(\d)(?=(\d{3})+(,|$))/g, '$1,')
						...
						[
							{ text: 'Fair Market Value:' },
							{
								text:
									'$' +
									subjectPropertyAdjustedPrice.toFixed().replace(/(\d)(?=(\d{3})+(,|$))/g, '$1,')
							}
						]
					]
				},
			...
			{
				text: 'Uniform and Equal Calculations',
				style: 'uandePage',
				fontSize: 20,
				marginBottom: 10,
				marginTop: 30,
			...
			{
				text:
					'We are protesting this tax appraisal on the basis of a Uniform and Equal appraisal. We are only debating IMPROVEMENT value per square foot. Land value and extra ' +
					...
					'grades, conditions, and desirability as determined by HCAD.',
				style: 'uandePage'
			},
			{
				text: 'We then make adjustments to the comparables against the subject property on 3 factors...',
				style: 'uandePage'
			},
			{
				ul: [
					'CAD (Cost & Design): This accounts for differences in what is typical for the neighborhood. For example, new remodels or a new home in an older neighborhood, etc. Not all properties will have a CAD value.',
					...
					'GRD (Grade): This is a score assigned by the appraisal district to account for the quality of workmanship and materials'
				],
				style: 'uandePage'
			},
			{
				text: '(((Comp Improvement Value * Comp CAD/CDU/GRD Factor) * Subject CAD/CDU/GRD Factor) - Comp Improvement Value) = CAD/CDU/GRD Adjustment $',
				style: 'uandePage'
			},
			{
				text:
					'We make these adjustments to each comparable IMPROVEMENT value and then divide by the sqaure footage to get an adjusted price per sq ft. ' +
					...
					'under the Uniform and Equal law in Section 41.43 of the Texas Property Tax Code.',
				style: 'uandePage'
			},
			/************ PAGE 3 - Comps Table ***************/
				...
				table: {
					...
					body: [
						[
							...
							{ text: '$' },
							{ text: compAvgSqFtPrice.toFixed(2) }
						],
						[
							{ text: 'Subject Imprv Value @ Median: ' },
							{ text: '$' },
							{
								text: (subjectProperty.sqFt * compAvgSqFtPrice)
									.toFixed()
									.replace(/(\d)(?=(\d{3})+(,|$))/g, '$1,')
							}
						],
						[
							{ text: '+ Extra Features: ' },
							{ text: '$' },
							{ text: subjectProperty.xFeatures.toFixed().replace(/(\d)(?=(\d{3})+(,|$))/g, '$1,') }
						],
						[
							{ text: '+ Land Value: ', border: [false, false, false, true] },
							{ text: '$', border: [false, false, false, true] },
							{
								text: subjectProperty.landValue.toFixed().replace(/(\d)(?=(\d{3})+(,|$))/g, '$1,'),
								border: [false, false, false, true]
							}
						],
						[
							{ text: '= Adjusted Market Value: ' },
							{ text: '$' },
							{
								text: subjectPropertyAdjustedPrice
									.toFixed()
									.replace(/(\d)(?=(\d{3})+(,|$))/g, '$1,')
							}
						]
